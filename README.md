...
```bash
hermes skills install 'https://cdn.jsdelivr.net/gh/Zhenxiangai/link-video-downloader-zhenxiangai@v1.2.3/SKILL.md' --category social-media --name wechat-archive --force --yes
```

该命令固定到发布标签，并通过 jsDelivr 分发完整 Skill 文件，避免 GitHub Raw 节点临时出现 429、503 或连接超时。
...
`v1.2.3` 只修复公开安装链路：安装入口改用固定标签的 jsDelivr CDN，并将公众号本地授权文件路径改用不会被 Hermes 误判为环境凭据的 `WECHAT_MP_AUTH_FILE`；旧的 `WECHAT_MP_TOKEN_FILE` 仍兼容。归档、下载和恢复逻辑没有变化。
