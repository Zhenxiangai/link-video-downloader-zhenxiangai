...
- Current WeChat may omit the legacy Official Account identity and session variables from public article HTML. Identity fallback must use only a prior manifest or captured article whose `source_url` exactly matches the requested link; never bind the most recently seen Official Account. On current Mac WeChat, a complete session may arrive in either (a) an exact-host HTTPS `/s` article request with a same-request Cookie, or (b) an exact-host HTTPS `/mp/relatedsearchword` request without Cookie but with all of `__biz`, `uin`, `key`, `pass_ticket`, `appmsg_token`, `mid`, `idx`, and `sessionid`. The local backend must reject every other cookie-less path and every incomplete request, persist only a sanitized refresh URI, store the session file with owner-only permissions, and never log query values or Cookie content. Legacy `profile_ext?action=home|getmsg` remains supported. Do not fall back to browser Cookie import or print session values.
- If a captured request is incomplete or replay still returns `no session`, close capture and mark the batch as requiring reauthentication or protocol investigation; do not ask the user to repeat unrelated `全部消息` scrolling. When replay succeeds, enumerate every page, require a strictly advancing offset, and persist the confirmed `biz` in the batch manifest on every page so pagination can resume after an in-memory mapping expires. Strip all session fields from saved public URLs and stop at inventory-only unless the user explicitly authorizes bulk download.
...
- Official Account image-only posts are valid only when they have zero verification markers, a non-generic title, an actual `js_content` container, and at least one successfully archived image. For legacy media URLs, normalize ASCII authority case and upgrade HTTP to HTTPS only when the result is exactly `mmbiz.qpic.cn` or `mmbiz.qpic.cn:80`; empty userinfo, empty ports, trailing-dot hosts, alternate port spellings, and every other authority remain rejected. Never relax the general HTTPS/host allowlist.
