...
> Hermes 新机稳定安装入口：<https://raw.githubusercontent.com/Zhenxiangai/link-video-downloader-zhenxiangai/v1.2.0/SKILL.md>。内部 Skill ID 继续使用 `wechat-archive`，已有用户可原位升级。
...
- 四个视频平台的真实单链接、唯一视频、逐字稿和 manifest 全部通过；公众号完成 325 条历史盘点及 3 篇正文样本归档，不宣称已下载全部正文。
- 隔离新 HOME 完成透明核心安装、自检与同 Job 授权续跑，并验证 `v1.2.0/SKILL.md` 固定链接。
